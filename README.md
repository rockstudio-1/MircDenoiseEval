...
```bash
git clone --recursive https://github.com/rockstudio-1/MircDenoiseEval.git
cd MircDenoiseEval
```
...
| Method  | Parameter name                     | Parameter symbol/option | Default value  | Parameter meaning                                                                     | Rationale                                                                                   |
| ------- | ---------------------------------- | ----------------------- | -------------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| **WT**  | Wavelet Type                       | `--wavelet`             | sym8           | Type of wavelet basis function, controls time-frequency decomposition characteristics | sym8 strikes a good balance between time-frequency localization and smoothness              |
|         | Decomposition Level                | `--level`               | 8              | Controls the depth of signal decomposition                                            | 8 levels effectively cover the main frequency range of microseismic signals (1–5 kHz)       |
|         | Threshold Type                     | `--threshold_type`      | soft threshold | Controls denoising intensity and signal fidelity                                      | While effectively suppressing noise, maintaining the continuity and integrity of the signal |
| **EMD** | IMF Energy Threshold Ratio         | `--imf_threshold`       | 0.2            | Energy threshold ratio for determining whether an IMF is noise                        | 0.2 effectively removes low-energy noise IMFs                                               |
|         | Number of Noise Modes to Remove    | `--noise_modes`         | auto           | Automatically determines the number of IMFs to remove                                 | Based on correlation between IMFs and the original signal, adaptive selection               |
|         | Maximum Number of Decomposed Modes | `--max_modes`           | None           | Limits the total number of IMFs                                                       | Default setting adapts to the complexity of different signals                               |
| **VMD** | Bandwidth Constraint Parameter     | `--alpha`               | 2000           | Controls the bandwidth of the modes                                                   | A higher alpha smooths the signal components                                                |
|         | Number of Modes                    | `--K`                   | 5              | Number of modes to extract                                                            | Balances frequency resolution with computational efficiency                                 |
|         | Mode Selection Criterion           | `--mode_selection`      | correlation    | Criterion for selecting modes to remove                                               | Correlation-based criterion is more suitable for engineering noise environments             |
